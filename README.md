...
Run `npm install`(or `npm i` para versão curta) para instalar as dependências dev

## Tests

Nesse projeto, você pode rodar os testes no desktop ou na visualização mobile

### Desktop

Run `npm test`(or `npm t` para a versão curta) para rodar o teste no modo headless ou na visualização desktop
or, run `npm cy:open` para abrir o cypress app no desktop

### Mobile
Run `npm run test:open` para rodar no modo headless na visualização mobile
Or run `run npm cy:open:mobile`para abrir o cypress app na visualização mobile
___
...
Essas foram as ferramentas e conteúdos usados para completar o curso
Este projeto está sendo feito por[Daniel](https://github.com/daniellpds260-crypto).
